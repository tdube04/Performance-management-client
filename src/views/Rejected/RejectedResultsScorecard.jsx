...
const RejectedResultsScorecard = () => {
  ...
  const getIncompleteAppraiseeWorkplan = async () => {
    try {
      ...
      for (let i = 0; i < appraisees.length; i++) {
        ...
        console.log("Appraisee scorecard Rejected");
  ...
  return (
    <div style={{ marginLeft: "100px" }}>
      ...
      >
        ...
        >
          Current Quater Rejected Results Scorecard for Appraisees
        </Typography>
      </Paper>
      {appraiseeProfileData && workPlanData && (
        <MaterialReactTable
          ...
          data={appraiseeProfileData
            ...
            .filter((appraisee) => appraisee.scorecardStatus === "Rejected")}
          ...
          muiTableBodyRowProps={({ row, index }) => ({
            onClick: (event) => {
              ...
              const workplanUrl = `/selected-rejected-appraisee-results-scorecard?${queryParams}`;
...
export default RejectedResultsScorecard;
